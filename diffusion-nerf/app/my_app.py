...
class MyApp(WispApp):
    ...
    def _init_scene_graph(self, wisp_state: WispState) -> None:
        ...
        wisp_state.graph.channels['normal'] = Channel(blend_fn=blend_normal,   # Ignore alpha blending
                                                      normalize_fn=normalize,  # Map to [0.0, 1.0]
                                                      min_val=-1.0, max_val=1.0)
        wisp_state.graph.channels['normal_pred'] = Channel(blend_fn=blend_normal,   # Ignore alpha blending
                                                           normalize_fn=normalize,  # Map to [0.0, 1.0]
                                                           min_val=-1.0, max_val=1.0)
    ...
    def _init_interactive_renderer_properties(self, wisp_state: WispState) -> None:
        ...
        wisp_state.renderer.canvas_width = 720
        wisp_state.renderer.canvas_height = 720
        ...
        wisp_state.renderer.available_canvas_channels = ["rgb", "depth", "normal", "normal_pred",
                                                         "albedo", "textureless", "lambertian", "background"]
